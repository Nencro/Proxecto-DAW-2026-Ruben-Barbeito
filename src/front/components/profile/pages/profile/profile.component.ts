...
export class ProfileComponent implements OnInit {
  ...
  get emailMostrado(): string {
    if (this.esPerfilPropio()) {
      return this.email;
    }

    return this.codificarEmail(this.email);
  }
  ...
  private esPerfilPropio(): boolean {
    const usuarioActual = this.servicioAuth.getUser();

    if (!usuarioActual?.userName || !this.userName) {
      return !this.adminMode;
    }

    return usuarioActual.userName.toLowerCase() === this.userName.toLowerCase();
  }

  private codificarEmail(email: string): string {
    ...
    const nombreDominio = dominioPartes.shift() || '';
    const extension = dominioPartes.length ? `.${dominioPartes.join('.')}` : '';

    return `${this.codificarParteEmail(usuario)}@${this.codificarParteEmail(nombreDominio)}${extension}`;
  }

  private codificarParteEmail(valor: string): string {
    if (valor.length <= 1) {
      return '*';
    }
    ...
    return `${valor[0]}${'*'.repeat(Math.max(3, valor.length - 2))}${valor[valor.length - 1]}`;
  }
}
